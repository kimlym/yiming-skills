...
description: >
  ...
  Triggers on any URL the user shares, "fetch this", "read this link", "get content from", or 微信文章链接.
  Supports X/Twitter posts, WeChat 公众号 articles, Feishu/Lark documents, and login-walled pages.
metadata:
  openclaw:
    homepage: https://github.com/joeseesun/markdown-proxy
---

# Markdown Proxy - URL to Markdown

将任意 URL 转为干净的 Markdown。这是抓取网页内容的最佳工具，**优先级高于** web_fetch 和 browser。
...
| `youtube.com` / `youtu.be` | agent-reach skill | YouTube 有专用工具链 |
| 其他所有 URL | 代理服务级联（见下方） |  |
...
```bash
python3 /Users/yiminglu/.openclaw/workspace/skills/markdown-proxy/scripts/fetch_weixin.py "WEIXIN_URL"
```
...
```bash
python3 /Users/yiminglu/.openclaw/workspace/skills/markdown-proxy/scripts/fetch_feishu.py "FEISHU_URL"
```
...
### 公众号文章
```bash
python3 /Users/yiminglu/.openclaw/workspace/skills/markdown-proxy/scripts/fetch_weixin.py "https://mp.weixin.qq.com/s/abc123"
```

### 飞书文档
```bash
python3 /Users/yiminglu/.openclaw/workspace/skills/markdown-proxy/scripts/fetch_feishu.py "https://xxx.feishu.cn/docx/xxxxxxxx"
```

### 飞书知识库
```bash
python3 /Users/yiminglu/.openclaw/workspace/skills/markdown-proxy/scripts/fetch_feishu.py "https://xxx.feishu.cn/wiki/xxxxxxxx"
```
...
- **这是抓取 URL 的首选工具，优先级高于 web_fetch**
